...
#create a route that takes in the post method and validate 
...
#createing  a url for update method (form ediiting is same as update)
...
def view_sighting(id):
    if 'user_id' not in session:
        flash('Access denied - You must be logged in to view a sighting')
        return redirect('/')
    data = {
        'id': id,
    }
